...
map<string, int> rel_doc_query;
map<map<string, int>, int> rel_doc;		// 문서가 몇번 쿼리와 적합한지 저장
map<int, int> relevant_num;			// 각 쿼리당 relevant 한 문서 수
map<int, int> relevant;		// 검색결과중 relevant한 문서 개수

void evaluator(int retrieval_doc_num)
{

	string temp;
	int temp1 = 0;

	ifstream rel_document("relevant_document.txt");		// relevant 문서 집합

	while(getline(rel_document, temp))
	{
		string queryId;
		int i;
		string docId;
		stringstream rel_inf;
		rel_inf << temp.c_str();
		getline(rel_inf, queryId, '\t');
		getline(rel_inf, docId, '\r');
		i = atoi(queryId.c_str());
		if(relevant_num.find(i) == relevant_num.end()){
			relevant_num[i] = 0;
		}
		relevant_num[i] += 1;			// relevant 문서 개수 map에 누적 저장
		rel_doc_query[docId] = i;		// 문서가 i번째 쿼리와 relevant
		rel_doc[rel_doc_query] = 1;		// map에 저장
		rel_doc_query.clear();
	}
	rel_document.close();
	...
	{
		if(temp[0] == 'W' && temp[1] == 'S' && temp[2] == 'J')
		{
			string docId;
			stringstream rel_inf;
			rel_inf << temp.c_str();
			while(getline(rel_inf, docId, '\t')){
				count++;
				rel_doc_query[docId] = n;	//n=쿼리번호
				int idx;
				if((idx = docId.find("\r")) != string::npos){
					docId = docId.substr(0, idx-1);
				}
				if(rel_doc[rel_doc_query])	// 검색결과중 relevant한 문서 발견
				{
					if(relevant[n])
					{
						relevant[n]++;
...
int main(){
	...
	while(getline(fin,strLine)){
		...
		if(strLine.find("<title>")==0){
			...
			if(!docTok.empty()){
				cout<<"------------ 쿼리문서 "<<querynum<<" 에 대한 벡터스페이스모델 만들기 시작--------------"<<endl;
...
//	ifstream wordTxt;
	...
	//쿼리의 tf를 구하기 위하여
	typedef pair<int,int>rel_key;
	map<string,float> QtfMap;
	map<string,float>::iterator Qit;
	map<Key,double> WMap;
	map<int,float> newQtfMap;
	map<int,float>::iterator newQit;
	map<rel_key,int> isRelDoc;
	float querySize=0;
	int docLen=doc.size();
...
//	top10_file<<endl;
	...
	string findWord;
	map<int,double> rel_feedback;//새로운 Qj저장할 곳
	int cntWord=0;
	int a=8;
	int b=16;
	int c=4;
	...
	for(Qit=QtfMap.begin();Qit!=QtfMap.end();Qit++){ //쿼리의 모든 단어에 대해
		...
		rel_feedback[cntWord]=a*Qit->second;
				...
				for(int i=0;i<df;i++){ //그 단어가 나온 문서갯수만큼 돌아야..
					...
					string docTemp=DocMap[docId];
					transform(docTemp.begin(),docTemp.end(),docTemp.begin(),toupper);
					rel_doc_query[docTemp]=querynum;
					if(rel_doc[rel_doc_query]){ //docTemp가 relevant한 문서야
					//	isRelDoc[rel_key(docId,cntWord)]=1;
					//	double plus=b*(double)(1/relevant[querynum])*docWeight;
					//	rel_feedback[cntWord]+=plus;
						rel_feedback[cntWord]+=b*((double)1.0/(double)relevant[querynum])*docWeight;
					}
					else{ //relevant하지 않아
					//	isRelDoc[rel_key(docId,cntWord)]=0;
					//	int rel_num=1000-relevant[querynum];
					//	double nonrel=(double)1.0/(double)rel_num;
					//	double minus=c*nonrel*docWeight;
					//	rel_feedback[cntWord]-=minus;
						rel_feedback[cntWord]-=c*((double)1.0/(double)(1000-relevant[querynum]))*docWeight;
					}
					rel_doc_query.clear();
				...
				querySize+=pow(rel_feedback[cntWord],2);
				cntWord++;
	...
	cout<<"유사도 계산 시작"<<endl;
		...
		for(Wit=WMap.begin();Wit!=WMap.end();Wit++){
			Dmap[Wit->first.first]+=Wit->second*rel_feedback[Wit->first.second]; //분자
	...
	for(Cos_it=Cos.rbegin();cnt<1000;Cos_it++){
		transform(DocMap[Cos_it->second].begin(),DocMap[Cos_it->second].end(),DocMap[Cos_it->second].begin(),toupper);
		if(cnt!=999)
			top10_file<<DocMap[Cos_it->second]<<"\t";//문서이름하고 웨이트를 저장
		else
			top10_file<<DocMap[Cos_it->second]<<endl;//문서이름하고 웨이트를 저장
		cnt++;
	}
